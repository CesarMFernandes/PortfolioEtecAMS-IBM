...
select sum(produtos.valor_unitario * itens_pedidos.quantidade) as valor_total_vendido
from itens_pedidos
inner join produtos on itens_pedidos.id_produto = produtos.id;

select unidade, avg(valor_unitario) as valor_medio from produtos where unidade like 'L';

select uf, count(*) as quant_clientes from clientes
group by uf;

select vendedores.nome, sum(itens_pedidos.quantidade * produtos.valor_unitario) as valor_vendido 
from vendedores
